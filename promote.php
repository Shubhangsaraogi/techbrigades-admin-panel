...
<body>
    <div class="container">
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="POST">
            ...
            ?>
            <h4>Select the promotion level of <?php echo $name; ?></h4>
    ...
    if (isset($_POST['submit'])) {
        if (!empty($_POST['level']) && isset($_POST['level'])) {
            ...
            if(!$result=mysqli_query($conn, $sql))
            {
                ?>
                <script>
                    location.replace("promote.php?msg=<?php echo $conn->error;?>");
                </script>
                <?php 
            }
            $temp=0;
            ...
            if($newlevel!=1)
            {
                ...
                ?>
            <script>
                location.replace("promote2.php?id=<?php echo $id;?>&newlevel=<?php echo $newlevel;?>");
                </script>
            <?php             
            }
            else
            {
                $sql = "DELETE FROM boss WHERE junior_id='$id'";
                if($result=mysqli_query($conn, $sql))
                {
                    ?>
                    <script>
                        location.replace("update.php?msg=Updated Successfull");
                    </script>
                    <?php 
                }
            }
