...
<input type="hidden" name="newlevel" value="<?php echo $newlevel;?>">
    ...
    if (isset($_POST['submit'])) {
        if (!empty($_POST['boss']) && isset($_POST['boss'])) {
            ...
            $sql = "UPDATE boss SET junior_level='$newlevel',boss_id='$boss_id',boss_level='$boss_level' WHERE junior_id='$id'";
            if(!$result=mysqli_query($conn, $sql))
            {
                ?>
                <script>
                    location.replace("promote2.php?msg=<?php echo $conn->error;?>");
                </script>
                <?php 
            }
